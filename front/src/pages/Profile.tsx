...
import { Gem, MapPin, Handshake, ChevronRightIcon, Package, Award, Flame, ReceiptText, Trophy } from 'lucide-react';
import { ProfileHeader } from '../components/ProfileHeader';

// --- Хелперы (остаются без изменений) ---
function getCategoryLabel(category: string): string {
  const categoryLabels: Record<string, string> = { 'accessories': 'Аксессуары', 'shoes': 'Обувь', 'clothes': 'Одежда', 'other': 'Другое' };
...
function formatRelativeTime(dateString?: string): string {
  ...
  try { return formatDistanceToNowStrict(new Date(dateString), { addSuffix: true, locale: ru }); } 
  catch (e) { return ''; }
}
...
export default function Profile() {
  ...
  const profile = useMemo(() => {
    ...
    return { ...rawProfile, address: rawProfile.address || { deliveryType: 'pickup', street: '', pickupAddress: '' } };
  }, [rawProfile]);
  ...
  if (isError) return <div className="flex items-center justify-center h-screen text-red-400 p-4 text-center">Ошибка загрузки профиля: {error instanceof Error ? error.message : 'Произошла неизвестная ошибка'}</div>;
  if (!baseUser || !profile) return null;

  return (
    <div className="relative min-h-screen bg-[#0a0a0a] text-white overflow-hidden max-w-screen-sm mx-auto no-scrollbar">
      <ProfileHeader />
      ...
      <div className="px-4" style={{ paddingTop: `calc(var(--tg-viewport-header-height) + 80px)` }}>
        ...
        <div className="flex flex-col gap-3 pb-20">
          ...
          <ProfileCard title="Награда за активность" subtitle="Ваши достижения в нашем сервисе" icon={<Award className="w-4 h-4 text-neutral-400" />}>
          ...
          <ProfileCard title="Адрес доставки (СДЭК)" 
            ...
            onClick={() => { haptic.light(); setAddressOpen(true); }} 
